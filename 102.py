...
current_task = 0
task_completed = False  # Flag to track completion
...
def update_progress(increment=1):
    ...
    global progress_percentage, current_task, task_completed
    ...
    print(f'\r[PROGRESS] [{bar}] {progress_percentage}%', end='', flush=True)
...
def display_hacking_progress():
    ...
    if random.random() < 0.3:  # 30% chance to show a message
        ...
        update_progress(0.2)  # Small progress increment
        time.sleep(0.1)
...
def display_account_hack(faculty_name):
    ...
    print(f"\033[91m{msg.format(faculty_name)}\033[0m")  # Red color
...
def estimate_completion_time(scopus_id, scholar_id):
    ...
    print(f"\n\033[93m[!] Estimated completion time: {time_str}\033[0m")
    print("\033[93m[!] Operation in progress. Do not interrupt...\033[0m\n")
...
def safe_get(url, headers=None, params=None, tries=3, timeout=12, sleep=0.6):
    ...
    for attempt in range(tries):
        try:
            ...
            update_progress(0.2)  # Small progress for each request
            return r
        except Exception:
            if attempt + 1 == tries:
...
def fetch_scopus_author_metrics(scopus_id):
    ...
    try:
        ...
        print(f"\033[91m[!] Error fetching Scopus author metrics: {e}\033[0m")
...
def fetch_scholar_author_metrics(scholar_id):
    ...
    try:
        ...
        if not author:
            print(f"\033[91m[!] Google Scholar author not found: {scholar_id}\033[0m")
            return {"h_index": "", "i10_index": "", "total_citations": ""}
        ...
        print(f"\033[91m[!] Error fetching Google Scholar metrics: {e}\033[0m")
        return {"h_index": "", "i10_index": "", "total_citations": ""}
...
def fetch_scopus_author_pubs(author_id, per_page=25, max_pages=None):
    ...
    while True:
        ...
        print(f"\033[94m[+] Fetching Scopus batch start={start} ...\033[0m")
        try:
            display_hacking_progress()
            r = safe_get(url, headers=headers, params=params, tries=3, sleep=0.6, timeout=20)
        except Exception as e:
            print(f"\033[91m[!] Scopus request failed: {e}\033[0m")
            break
        if not r or r.status_code != 200:
            print(f"\033[91m[!] Scopus API returned {getattr(r,'status_code',None)}\033[0m")
            try:
        ...
        try:
            ...
            print(f"\033[91m[!] Scopus response not JSON (skipping):\033[0m", getattr(r, "text", "")[:200])
            break
    ...
    print(f"\033[92m[✔] Total Scopus entries fetched: {len(all_entries)}\033[0m")
...
def fetch_scholar_author_pubs(scholar_id):
    if not scholarly:
        print(f"\033[91m[!] scholarly library not installed. Install: pip install scholarly\033[0m")
        return None
    try:
        print(f"\033[94m[+] Fetching Google Scholar author {scholar_id} ...\033[0m")
        display_hacking_progress()
        author = scholarly.search_author_id(scholar_id)
        ...
        print(f"\033[91m[!] Scholar fetch error: {e}\033[0m")
        return None
...
def save_outputs(faculty_name, rows):
    ...
    print(f"\033[94m[+] Writing data to {csv_fn}...\033[0m")
    ...
    print(f"\033[94m[+] Writing data to {json_fn}...\033[0m")
...
def select_faculty():
    print("\n\033[96m" + "="*60 + "\033[0m")
    print("\033[96m           MSRIT ACADEMIC DATA EXTRACTION TOOL\033[0m")
    print("\033[96m" + "="*60 + "\033[0m\n")
    print("Select faculty to extract data:\n")
    for i, f in enumerate(FACULTY, start=1):
        print(f"\033[93m{i:2d}. {f['name']}\033[0m")
    print("\n\033[91m 0. Exit\033[0m")
    try:
        num = int(input("\n\033[92mEnter number: \033[0m").strip())
    except Exception:
        return None
...
def main():
    ...
    print(f"\n\033[94m[+] Target Acquired: {author_name}\033[0m")
    print(f"\033[94m[+] Scopus ID: {scopus_id or 'N/A'}\033[0m")
    print(f"\033[94m[+] Scholar ID: {scholar_id or 'N/A'}\033[0m")
    ...
    print("\n\033[96m[+] Extracting author metrics...\033[0m")
    author_metrics = get_combined_author_metrics(scopus_id, scholar_id)
    print(f"\033[92m[✔] Author Metrics Extracted:\033[0m")
    print(f"    \033[92mh-index: {author_metrics.get('h_index', 'N/A')}\033[0m")
    print(f"    \033[92mi10-index: {author_metrics.get('i10_index', 'N/A')}\033[0m")
    print(f"    \033[92mTotal Citations: {author_metrics.get('total_citations', 'N/A')}\033[0m")
    if author_metrics.get('document_count'):
        print(f"    \033[92mDocument Count: {author_metrics.get('document_count')}\033[0m")
    print()
    ...
    if scholar_id and scholarly:
        try:
            print("\033[96m[+] Accessing Google Scholar database...\033[0m")
            scholar_filled = fetch_scholar_author_pubs(scholar_id)
            if scholar_filled:
                print("\033[92m[✔] Scholar publications extracted.\033[0m")
        except Exception as e:
            print(f"\033[91m[!] Scholar extraction failed: {e}\033[0m")
    elif not scholarly:
        print("\033[91m[!] scholarly not installed; skipping scholar data.\033[0m")

    # Fetch Scopus entries
    scopus_entries = []
    if scopus_id:
        try:
            print("\033[96m[+] Penetrating Scopus API...\033[0m")
            scopus_entries = fetch_scopus_author_pubs(scopus_id, per_page=25)
        except Exception as e:
            print(f"\033[91m[!] Scopus extraction failed: {e}\033[0m")

    # Build unified rows
    print("\033[96m[+] Compiling extracted data...\033[0m")
    rows = build_unified_rows(author_name, scholar_filled, scopus_entries, scopus_id, scholar_id)

    if not rows:
        print("\033[91m[!] No data extracted. Mission failed.\033[0m")
        return

    # Save outputs with faculty name
    print("\033[96m[+] Writing exfiltrated data to disk...\033[0m")
    csv_file, json_file = save_outputs(author_name, rows)
    
    # Ensure progress reaches exactly 100%
    if progress_percentage < 100:
        update_progress(100 - progress_percentage)
    ...
    print(f"\033[92m[✔] Estimated time: {estimated_time}s\033[0m")
    print(f"\033[92m[✔] Actual time: {actual_time}s\033[0m")
    print(f"\033[92m[✔] Total records extracted: {len(rows)}\033[0m")
    print(f"\033[92m[✔] CSV file saved as: {csv_file}\033[0m")
    print(f"\033[92m[✔] JSON file saved as: {json_file}\033[0m")
    print(f"\033[92m" + "="*60 + "\033[0m")
    print(f"\033[91m[!] Covering tracks... Data extraction complete!\033[0m")
    print(f"\n\033[93m[⚠] IMPORTANT NOTE:\033[0m")
    print(f"\033[93m[⚠] Data scraped through APIs (Scopus & Google Scholar) may contain inaccuracies.\033[0m")
    print(f"\033[93m[⚠] Please verify critical information before use in official reports.\033[0m")
    print(f"\033[93m[⚠] API limitations may affect completeness and accuracy of extracted data.\033[0m\n")

if __name__ == "__main__":
    try:
        main()
    except KeyboardInterrupt:
        print("\n\033[91m[!] Operation aborted by user. Self-destruct initiated...\033[0m")
    except Exception as e:
